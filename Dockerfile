...
RUN addgroup -S cloudsync && adduser -S cloudsync -G cloudsync

WORKDIR /app

# Create directories
RUN mkdir -p /tmp/cloudsync-uploads && chown -R cloudsync:cloudsync /tmp/cloudsync-uploads
